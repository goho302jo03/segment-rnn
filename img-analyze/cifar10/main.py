...
class Cifar10(Pipeline):
    def __init__(self):
        ...
        self.nb_classes = 10
        self.nb_units = 50
        self.img_rows, self.img_cols = 70, 96
        self.batch_size = 128
    ...
    def reshape_cifar10(self, x):
        return np.asarray([np.concatenate((v[:,:,0], v[:,:,1], v[:,:,2]), axis=1) for v in x])


    def data_loader(self):
        (tr_x, tr_y), (te_x, te_y) = cifar10.load_data()
        tr_x, te_x = map(self.reshape_cifar10, [tr_x, te_x])
    ...
    def build_model(self):
        model = Sequential()
        model.add(self.neuron_type(self.nb_units, input_shape=(None, self.img_cols), dropout=0.2, recurrent_dropout=0.2, return_sequences=True))
        model.add(self.neuron_type(self.nb_units, dropout=0.2, recurrent_dropout=0.2, return_sequences=True))
